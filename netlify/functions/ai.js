...
const TASKS = {
  ask: {
    ...
    model: 'claude-sonnet-4-6', max: 800, pro: true
  },
  scout: {
...
async function userTier(supaUrl, serviceKey, userId) {
  try {
    const r = await fetch(supaUrl + '/rest/v1/gwedge_profiles?user_id=eq.' + encodeURIComponent(userId) + '&select=tier',
      { headers: { apikey: serviceKey, Authorization: 'Bearer ' + serviceKey } });
    if (!r.ok) return 'free';
    const rows = await r.json();
    return rows && rows[0] && rows[0].tier === 'pro' ? 'pro' : 'free';
  } catch (_) { return 'free'; }
}
