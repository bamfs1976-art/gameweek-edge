...
ok(/<title>Gameweek Edge articles \| Gameweek Edge<\/title>/.test(idx) && /rel="canonical" href="https:\/\/gameweekedge\.co\.uk\/blog\/"/.test(idx) && BLOG_BASE === '/blog/', 'the index has a title and its canonical at /blog/');
ok(/rel="alternate" type="application\/rss\+xml"/.test(idx) && /href="\/blog\/second-post\/">The second post</.test(idx) && /10 September 2026/.test(idx), 'it links the feed and every post with its date');
const emptyIdx = renderIndex([], L);
ok(/No articles yet/.test(emptyIdx) && /href="\/wire">The Wire</.test(emptyIdx) && /href="\/tools\/"/.test(emptyIdx), 'with no post the index says so and points at The Wire and the tools');
...
ok(/^<\?xml version="1.0" encoding="UTF-8"\?>\n<rss version="2.0"/.test(feed) && (feed.match(/<item>/g) || []).length === 2 && /<guid isPermaLink="true">https:\/\/gameweekedge\.co\.uk\/blog\/first-post\/<\/guid>/.test(feed), 'RSS 2.0 with an item per post and permalink guids');
ok(/<rss version="2.0"/.test(renderFeed([], '2026-09-12T12:00:00Z')) && !/<item>/.test(renderFeed([], '2026-09-12T12:00:00Z')), 'an empty feed is still a valid channel');
ok(/<atom:link href="https:\/\/gameweekedge\.co\.uk\/blog\/feed\.xml" rel="self"/.test(feed) && /<language>en-gb<\/language>/.test(feed) && /<pubDate>Tue, 01 Sep 2026 09:00:00 GMT<\/pubDate>/.test(feed), 'a self link, the language and RFC dates');
...
section('the build writes the set, or says why not');
{
  ...
  ok(res.posts === 2 && res.urls.length === 3 && existsSync(join(out, 'blog', 'index.html')) && existsSync(join(out, 'blog', 'feed.xml')) && existsSync(join(out, 'blog', 'first-post', 'index.html')), 'index, feed and a page per post, with sitemap entries');
  const empty = mkdtempSync(join(tmpdir(), 'ge-blog-empty-'));
  const none = await buildBlog(empty, out, L);
  ok(none.posts === 0 && none.urls.length === 1 && /no published post/.test(none.note) && existsSync(join(out, 'blog', 'index.html')) && existsSync(join(out, 'blog', 'feed.xml')), 'no published post still writes the index and the feed, and says why nothing else');
